...
class HowManyYearsController @Inject() (
    ...
    (identify andThen featureActions.ftnaeAction andThen getData andThen requireData).async { implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(HowManyYearsPage, value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(HowManyYearsPage, mode, updatedAnswers))
        )
    }
}
