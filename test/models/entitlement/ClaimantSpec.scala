  ...
  "Claimant" - {
      ...
      forAll(trueFalseCases) { withAdjustment =>
        s"AND an adjustment information ${isOrIsNot(withAdjustment)} provided" - {
          "THEN the expected Claimant is returned" in {
            forAll(arbitrary[FullName], arbitrary[BigDecimal], arbitrary[LocalDate],
              arbitrary[Seq[LastPaymentFinancialInfo]], arbitrary[FullAddress], arbitrary[AdjustmentInformation]
            ) {
              (name, decimalValue, date, lastPayments, fullAddress, adjustmentInformation) =>
                val result = Claimant(
                  name, decimalValue, date, date, decimalValue, decimalValue, lastPayments, fullAddress,
    ...
    "format: should successfully format to JSON" in {
      forAll(arbitrary[FullName], arbitrary[BigDecimal], arbitrary[LocalDate],
        ...
        (name, decimalValue, date, lastPayments, fullAddress, adjustmentInformation) =>
          val claimant = Claimant(name, decimalValue, date, date, decimalValue, decimalValue, lastPayments, fullAddress, Some(adjustmentInformation))
