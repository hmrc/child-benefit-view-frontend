  ...
  "FullAddress" - {
      ...
      forAll(trueFalseCases) { withAddress3 =>
        forAll(trueFalseCases) { withAddress4 =>
          forAll(trueFalseCases) { withAddress5 =>
            s"AND a third address line ${isOrIsNot(withAddress3)} provided - a fourth address line ${isOrIsNot(withAddress4)
            } provided - a fifth address line ${isOrIsNot(withAddress5)} provided" - {
              "THEN the expected FullAddress is returned" in {
                forAll(arbitrary[AddressLine], arbitrary[AddressLine], arbitrary[AddressLine],
                  arbitrary[AddressLine], arbitrary[AddressLine], arbitrary[AddressPostcode]) {
    ...
    "format: should successfully format to JSON" in {
      forAll(arbitrary[AddressLine], arbitrary[AddressLine], arbitrary[AddressLine],
        arbitrary[AddressLine], arbitrary[AddressLine], arbitrary[AddressPostcode]) {
        (addressLine1, addressLine2, addressLine3, addressLine4, addressLine5, addressPostcode) =>
          val fullAddress = FullAddress(addressLine1, addressLine2, Some(addressLine3), Some(addressLine4), Some(addressLine5), addressPostcode)
          Json.toJson(fullAddress)
      }
    }
    "toSingleLineString" - {
      def expectedSingleLineString(
        fullAddress: FullAddress, withAddress3: Boolean, withAddress4: Boolean, withAddress5: Boolean
      ): String =
        s"${fullAddress.addressLine1.value} ${fullAddress.addressLine2.value} " +
          s"${if(withAddress3) s"${fullAddress.addressLine3.fold("-Missing Expected Address Line: 3")(_.value)} " else ""}" +
          s"${if(withAddress4) s"${fullAddress.addressLine4.fold("-Missing Expected Address Line: 4")(_.value)} " else ""}" +
          s"${if(withAddress5) s"${fullAddress.addressLine5.fold("-Missing Expected Address Line: 5")(_.value)} " else ""}" +
          s"${fullAddress.addressPostcode.value}"
      "GIVEN a valid first address line, second address line and an address postCode" - {
        forAll(trueFalseCases) { withAddress3 =>
          forAll(trueFalseCases) { withAddress4 =>
            forAll(trueFalseCases) { withAddress5 =>
              s"AND a third address line ${isOrIsNot(withAddress3)} provided - a fourth address line ${
                isOrIsNot(withAddress4)
              } provided - a fifth address line ${isOrIsNot(withAddress5)} provided" - {
                "THEN the expected FullAddress is returned" in {
                  forAll(arbitrary[AddressLine], arbitrary[AddressLine], arbitrary[AddressLine],
                    arbitrary[AddressLine], arbitrary[AddressLine], arbitrary[AddressPostcode]) {
    ...
    "toPageDisplayString" - {
      def expectedPageDisplayString(
        ...
        s"${fullAddress.addressLine1.value}</br>${fullAddress.addressLine2.value}</br>" +
          s"${if(withAddress3) s"${fullAddress.addressLine3.fold("-Missing Expected Address Line: 3")(_.value)}</br>" else ""}" +
          s"${if(withAddress4) s"${fullAddress.addressLine4.fold("-Missing Expected Address Line: 4")(_.value)}</br>" else ""}" +
          s"${if(withAddress5) s"${fullAddress.addressLine5.fold("-Missing Expected Address Line: 5")(_.value)}</br>" else ""}" +
                ...
                "THEN the expected FullAddress is returned" in {
                  forAll(arbitrary[AddressLine], arbitrary[AddressLine], arbitrary[AddressLine],
                    ...
                    (addressLine1, addressLine2, addressLine3, addressLine4, addressLine5, addressPostcode) =>
                      ...
                      val expectedResult = expectedPageDisplayString(fullAddress, withAddress3, withAddress4, withAddress5)
