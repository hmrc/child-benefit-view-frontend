...
class CBDataRetrievalActionSpec extends BaseAppSpec with MockitoSugar {

  class Harness(sessionRepository: SessionRepository) extends CBDataRetrievalActionImpl(sessionRepository) {
    def callTransform[A](request: IdentifierRequest[A]): Future[OptionalDataRequest[A]] = transform(request)
  }

  "CB Data Retrieval Action" - {
    ...
    "when there is data in the cache" - {
      ...
      "must build a userAnswers object and add it to the request" in {
        ...
        result.userAnswers.value.id mustBe "id"
      }
    }
  }
}
